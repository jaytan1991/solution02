...
FROM jobs Jobs
INNER JOIN job_categories JobCategories ON/*move the inner join to limit the result before left join*/
		(JobCategories.id = (Jobs.job_category_id)
	AND	(JobCategories.deleted) IS NULL)
	AND	publish_status = 1/*move the and clause from where clause to here to limite the result before left join*/
